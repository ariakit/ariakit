...
| `ak-layer-transparent`    | Paints only when a modifier or a state variant moves the layer color. At rest the element keeps its color context for descendants without painting a background, so whatever sits behind it stays visible.                                                                                       |
