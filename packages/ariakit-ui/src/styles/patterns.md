...
A translucent edge takes the colour of whatever it is painted over, so a line between two rows of different colours can only match one of them, and which one must not depend on paint order. Each table cell draws only the lines after it, the line below its row and the divider after its column, as borders over the row's own surface, and a row in a state, hovered or selected, takes the line above it as well: its cells paint that pixel with a two-part `box-shadow`, the row's surface and its edge over it, and a later sibling's shadow lands over an earlier sibling's border in tree order without any z-index. So a plain row's lines are the seam of its band, a state row's lines are the seam of its tint on all four sides, and no cell is lifted above its neighbours. Real borders make the padding box the box inside the lines, which is what a ring wants: a cell's ring is an inset ring, and it sits one line inside the cell on every side without any geometry of its own. A row's ring cannot be an outline, which paints under a pinned cell in every engine and keeps one offset on every side while the row holds the line below it and not the one above; it is a pseudo-element of the row, over the cells and under a sticky row group, inset by the lines the row's cells hold.
