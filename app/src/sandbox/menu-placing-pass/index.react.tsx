...
/**
 * Holds positioning open while the active item's DOM node is replaced. Stable
 * ids let the replacement remain the same logical item in the collection.
 */
function ReplacementMenu() {
  const menu = Ariakit.useMenuStore();
  const releaseRef = useRef<(() => void) | null>(null);
  ...
  return (
    <section style={{ display: "grid", gap: 8, justifyItems: "start" }}>
      <h2>Replacement actions</h2>
      <button
        id="replacement-actions-escape-target"
        type="button"
        tabIndex={0}
        onClick={menu.show}
      >
      ...
      <button
        id="replace-replacement-actions-items"
        type="button"
        tabIndex={0}
        onClick={() => setGeneration((value) => value + 1)}
      >
        Replace Replacement actions items
      </button>
      <button
        type="button"
        tabIndex={0}
        onClick={() => menu.move(`replacement-action-${actionCount}`)}
      >
        Move to last Replacement actions action
      </button>
      <button
        type="button"
        tabIndex={0}
