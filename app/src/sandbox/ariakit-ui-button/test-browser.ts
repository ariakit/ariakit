...
withCaptures(import.meta.dirname, async ({ query, test }) => {
  ...
  test(
    "page @visual",
    { annotation: { type: "ariviso:item", description: "ui/button/page" } },
    async ({ page, visual }) => {
  ...
  test(
    "shows the thick focus ring on keyboard focus @visual",
    {
      annotation: {
        type: "ariviso:item",
        description: "ui/button/shows-the-thick-focus-ring-on-keyboard-focus",
      },
    },
    async ({ page, q, visual }) => {
  ...
  test(
    "moves the segmented control glider to the clicked radio @visual",
    {
      annotation: {
        type: "ariviso:item",
        description:
          "ui/button/moves-the-segmented-control-glider-to-the-clicked-radio",
      },
    },
    async ({ page, q, visual }) => {
      await forEachColorScheme(page, async (colorScheme) => {
  ...
  test(
    "moves the hover glider over a hovered link @visual",
    {
      annotation: {
        type: "ariviso:item",
        description: "ui/button/moves-the-hover-glider-over-a-hovered-link",
      },
    },
    async ({ page, q, visual }) => {
      await forEachColorScheme(page, async (colorScheme) => {
        ...
        // apply :hover in WebKit until the pointer enters another element, so
        // the pointer passes over Settings on its way to Activity.
  ...
  for (const [key, title] of [
    ...
    test(
      `tints both shared edges of the hovered middle control in ${title} @visual`,
      {
        annotation: {
          type: "ariviso:item",
          description: `ui/button/joined-hover-${key}`,
        },
      },
      async ({ page, q, visual }) => {
        await forEachColorScheme(page, async (colorScheme) => {
