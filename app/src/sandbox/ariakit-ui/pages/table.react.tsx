...
const sortedByNameRows: TableRows<ComponentColumn> = [
  {
    ...
    component: {
      children: "Component",
      sort: "ascending",
      // The sort button at the table's top-start corner is the page's focus
      // target, so the screenshots show its ring following the rounding.
      sortButton: { ...screenshotFocus },
    },
...
markExampleCodeTransparent(PreScrolled);
...
// The snippets below describe declarative tables whose knob lives inside
// `rows`, which a snippet cannot print. Each one writes the composed parts the
// rows turn into, with one body row standing for all of them.

const selectedRowsCode = (
...
const sortedByVariantsCode = (
  <Table container={{ $border: true }}>
    <TableRowGroup group="head">
      <TableRow>
        <TableCell sort="none">Component</TableCell>
        <TableCell>Status</TableCell>
        <TableCell numeric sort="descending">
          Variants
        </TableCell>
      </TableRow>
    </TableRowGroup>
...
const reviewCode = (
  <Table container={{ $border: true }}>
    ...
    <TableRowGroup group="foot">
      <TableRow>
        <TableCell header="row" numeric $grow>
          Total
        </TableCell>
        <TableCell numeric $fit>
          8
