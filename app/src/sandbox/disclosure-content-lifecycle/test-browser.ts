...
withFramework(import.meta.dirname, async ({ query, test }) => {
  ...
  test("closes content with a truthy animated option and no CSS animation", async ({
    ...
    const section = q.region("Boolean animated");
    const withinSection = query(section);
    const content = withinSection.text("Boolean animated content");
    const toggle = withinSection.button("Toggle Boolean animated");

    await test.expect(content).not.toBeAttached();

    // The controlled animated option re-asserts true after enter detection
    // finds no CSS animation and turns the animated state off. Closing must
    // still stop the animation and unmount the content in that case.
    await toggle.click();
    await test.expect(content).toBeVisible();
    await test.expect(content).toHaveAttribute("data-enter", "true");
    await test.expect(section).toHaveAttribute("data-animated-state", "true");
  ...
  test("keeps stale data-enter off visible closed content when animations are re-enabled", async ({
    ...
    const section = q.region("Always visible");
    const withinSection = query(section);
    const content = withinSection.text("Always visible content");
    const toggle = withinSection.button("Toggle Always visible");

    // The content is visible even while closed (alwaysVisible). Open it so
    // animation detection turns animations off, then close it.
    await test.expect(content).toBeVisible();
    await toggle.click();
    await test.expect(content).toHaveAttribute("data-enter", "true");
    await toggle.click();
    ...
    // Re-enabling animations at runtime must not resurface the enter state
    // from the previous open on the closed, visible content, not even
    // transiently, so we record attribute mutations instead of sampling the
    // final state.
    const enterAppeared = await page.evaluate(async () => {
      const content = document.querySelector(".always-visible-content");
      if (!content) {
        throw new Error("Content not found");
      }
      const buttons = Array.from(document.querySelectorAll("button"));
      const animateButton = buttons.find(
        (button) => button.textContent === "Animate Always visible",
      );
      if (!animateButton) {
      ...
      const observer = new MutationObserver((mutations) => {
        for (const mutation of mutations) {
          ...
          if (!(mutation.target as Element).hasAttribute("data-enter")) {
            continue;
          }
          appeared = true;
        }
      ...
      animateButton.click();
      // Give the afterPaint (double requestAnimationFrame) path time to run.
    ...
    test.expect(enterAppeared).toBe(false);
    await test.expect(section).toHaveAttribute("data-animated-state", "300");
    await test.expect(content).not.toHaveAttribute("data-enter");
