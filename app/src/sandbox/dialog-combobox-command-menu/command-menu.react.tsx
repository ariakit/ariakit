  ...
  function CommandMenu({ open, onOpenChange, onSearch, ...props }, ref) {
    ...
    return (
      <Ariakit.Dialog
        ...
        unmountOnHide
        backdrop={
          <div className="fixed inset-0 z-40 opacity-0 transition-opacity data-enter:opacity-100" />
        }
        {...props}
        store={dialog}
        style={{ opacity: 1, transition: "opacity 150ms", ...props.style }}
        className={clsx(
          "relative z-50 opacity-0 transition-opacity data-enter:opacity-100",
          props.className,
        )}
      >
