---
title: Styling
description: Ariakit React provides unstyled components by default. You're free to style them using your favorite styling method.
---
...
> [!WARNING] Do not use any selectors that are not listed below
>
...
The `data-selected` attribute is applied to selected items that use the `CompositeSelectable` behavior. It is available for every role, including roles that do not support `aria-selected` or `aria-checked`.
