...
test.only("should return valid registry index", async ({ page }) => {
  ...
  const uiItems = data.items.filter((item) => item.type === "registry:ui");
  const libItems = data.items.filter((item) => item.type === "registry:lib");
...
test("should return valid UI registry item (react-aria)", async ({ page }) => {
...
test("style registry items should have valid CSS format", async ({ page }) => {
...
test("block items should include data files when present", async ({ page }) => {
  const indexResponse = await page.goto("/r/registry.json");
  const indexData = await indexResponse?.json();
...
test("block items transform _lib imports to registry paths", async ({
  ...
  expect(indexFile.content).toContain("@/registry/ui/ariakit-react-disclosure");
...
test("hook imports should include extensions with ?ext=true", async ({
  ...
  expect(mainFile.content).toContain('from "@/registry/hook/use-is-mobile.ts"');
});
