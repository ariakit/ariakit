...
Combobox groups provide visual separation and improved accessibility by grouping related options together. Each group can have its own header, making the interface more intuitive and scannable.

import ContentCodeBlock from "#app/components/content-code-block.astro";

<test>
  dsadsa <div slot="dsadsa">dsadsa</div>
</test>

<ContentCodeBlock
  persistTabKey="package-manager"
  tabs={[
    { filename: "npm", code: `npm install @ariakit/react` },
    { filename: "pnpm", code: `pnpm add @ariakit/react` },
    { filename: "yarn", code: `yarn add @ariakit/react` },
    { filename: "bun", code: `bun add @ariakit/react` },
  ]}
/>

```test.jsx {0-2} "label"0-2 'value="jp">J' "Com"
...
- **Better Visual Hierarchy**
  ...
  Visual grouping creates clear sections that help users understand the relationship between options and quickly scan for relevant categories.
...
- [x] Install the `@ariakit/react` package
- [x] Import `ComboboxGroup` component
- [ ] ~~Use deprecated `Select` component~~ (Use `Combobox` instead)
...
Users can navigate the combobox using these keyboard shortcuts:
...
| <kbd>Ctrl</kbd> + <kbd>A</kbd> | Select all text in input       |
...
<details>
<summary>View complete dynamic grouping example</summary>

```jsx
import { useMemo } from "react";

function DynamicGroupedCombobox({ data }) {
  ...
  return (
    <Combobox>
      {Object.entries(groupedData).map(([groupName, items]) => (
        <ComboboxGroup key={groupName} label={groupName}>
          {items.map((item) => (
...
When calculating optimal group sizes, consider the Miller's Rule: 7±2 items per group for optimal cognitive processing[^1].
...
**Issue**: Groups not appearing properly
**Solution**: Ensure you're wrapping items in `ComboboxGroup` components

**Issue**: Screen reader not announcing groups
**Solution**: Verify that group labels are properly set and not empty

**Issue**: Performance issues with large datasets
**Solution**: Implement virtualization or lazy loading as described above

</details>

---

_For more advanced examples and API documentation, check out the [complete component reference](./api)._

[^1]: Miller, George A. "The magical number seven, plus or minus two: Some limits on our capacity for processing information." _Psychological review_ 63.2 (1956): 81-97.
