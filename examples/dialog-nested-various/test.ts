...
test.each([
  "nested",
  "sibling",
])("show %s dialog and hide with escape", async (name) => {
  await click(q.button("Open dialog"));
...
test.each([
  "nested",
  "sibling",
])("show %s unmount dialog and hide with escape", async (name) => {
  await click(q.button("Open dialog"));
...
test.each([
  "nested",
  "sibling",
])("show %s no portal dialog and hide with escape", async (name) => {
  const maybeNoRole =
...
test.each([
  "nested",
  "sibling",
])("show %s no portal portal dialog and hide with escape", async (name) => {
  const maybeNoRole =
    name === "nested" ? q.none.includesHidden : q.dialog.includesHidden;
  await click(q.button("Open dialog"));
  ...
  expectModalStyle(true);
  await press.Escape();
  expect(q.dialog.includesHidden(`${name} no portal portal`)).not.toBeVisible();
...
test.each([
  "nested",
  "sibling",
])("show %s no backdrop dialog and hide by clicking outside", async (name) => {
  await click(q.button("Open dialog"));
...
test.each([
  "nested",
  "sibling",
])("show %s dismiss dialog and hide with escape", async (name) => {
  await click(q.button("Open dialog"));
...
test.each([
  "sibling",
])("show %s dismiss unmount dialog and hide with escape", async (name) => {
  await click(q.button("Open dialog"));
