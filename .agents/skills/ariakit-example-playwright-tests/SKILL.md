...
- Run those `test-browser.ts` files with the scripts in `site/package.json` via workspace commands such as `pnpm -F site run test`, `pnpm -F site run test-chrome`, the `-headed` variants, and the `test-visual*` scripts when needed.
