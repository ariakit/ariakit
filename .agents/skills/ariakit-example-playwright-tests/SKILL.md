...
- Run those `test-browser.ts` files with the existing npm scripts from the repository root, such as `npm run test-browser`, `npm run test-chrome`, and their `-headed` / `-debug` variants, optionally passing an example path filter when needed.
