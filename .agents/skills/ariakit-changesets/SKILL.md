...
- For changesets with multiple paragraphs or examples, the first paragraph **must** be a short title (not a full sentence). Title rules: it should not end with a period, should not contain links, and should be concise (e.g., `Fixed \`FormRadio\` registering to ancestor composite stores`). Past tense is allowed but not required (unlike single-paragraph changesets, which must use past tense). Inline code is okay. The detailed explanation goes in subsequent paragraphs, where links and full sentences are expected.
