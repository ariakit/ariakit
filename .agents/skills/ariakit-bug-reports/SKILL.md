...
- Start by reproducing the bug in a sandbox under `site/src/sandbox/<issue-number>/` and add an automated test that fails for the reported behavior. Most bug reports include a StackBlitz URL with the user's reproduction. You should always open that URL and review the code for reference. To run the browser tests locally, start the dev server with `pnpm dev-site` (faster than building) and then run `pnpm -F site run test-chrome -- site/src/sandbox/<issue-number>/`. At this point, you should commit your changes and create a draft PR so we can see the failed CI checks. Then start working on the workaround.
