...
[`Combobox`](https://ariakit.com/reference/combobox) is now also exported as `ComboboxInput` (with `ComboboxInputOptions` and `ComboboxInputProps` type aliases), a clearer name when the input is a filter inside the popover rather than the main combobox element.
