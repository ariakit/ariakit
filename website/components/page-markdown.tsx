...
export function PageMarkdown({
  ...
  const element = ReactMarkdown({
    ...
    rehypePlugins: [rehypeCodeMeta, rehypeRaw, rehypeSlug, rehypeWrapHeadings],
    components: {
      div({ node, ...props }) {
      ...
      p({ node, ...props }) {
        const paragraph = (
          <p {...props} className={twJoin(style.paragraph, props.className)} />
        );
